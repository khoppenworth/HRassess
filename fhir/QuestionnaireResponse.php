...
if ($method === 'GET') {
  ...
  if ($questionnaire) {
    $stmt = $pdo->prepare("SELECT id, created_at FROM questionnaire_response WHERE questionnaire_id = ? ORDER BY id DESC");
    $stmt->execute([(int)$questionnaire]);
    $entries = [];
    foreach ($stmt as $r) {
      $entries[] = ["resource"=>["resourceType"=>"QuestionnaireResponse","id"=>(string)$r["id"],"authored"=>$r["created_at"]]];
    }
